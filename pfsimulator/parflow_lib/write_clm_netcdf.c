    ...
    *myVarNCData = malloc(sizeof(varNCData));
    ...
    if (res != NC_ENAMEINUSE)
    {
      ...
      if (strcmp(switch_name, default_val) != 0)
      {
        ...
        chunksize[1] = GetInt("NetCDF.ChunkY");
        chunksize[2] = GetInt("NetCDF.ChunkX");
        nc_def_var_chunking(clmIDs[0], sndVarID, NC_CHUNKED, chunksize);
      }
    }
    if (res == NC_ENAMEINUSE)
    {
      res = nc_inq_varid(clmIDs[0], varName, &sndVarID);
    }
    return sndVarID;
  }

  if (strcmp(varName, "snoalb_out") == 0)
  {
    *myVarNCData = malloc(sizeof(varNCData));
    (*myVarNCData)->varName = varName;
    (*myVarNCData)->ncType = NC_DOUBLE;
    ...
    int snoalbVarID;
    int res = nc_def_var(clmIDs[0], varName, (*myVarNCData)->ncType, (*myVarNCData)->dimSize,
                         (*myVarNCData)->dimIDs, &snoalbVarID);
    if (res != NC_ENAMEINUSE)
    {
      char *switch_name;
      char key[IDB_MAX_KEY_LEN];
      ...
      if (strcmp(switch_name, default_val) != 0)
      {
        ...
        chunksize[1] = GetInt("NetCDF.ChunkY");
        chunksize[2] = GetInt("NetCDF.ChunkX");
        nc_def_var_chunking(clmIDs[0], snoalbVarID, NC_CHUNKED, chunksize);
      }
    }
    if (res == NC_ENAMEINUSE)
    {
      res = nc_inq_varid(clmIDs[0], varName, &snoalbVarID);
    }
    return snoalbVarID;
  }

  if (strcmp(varName, "surfalb_out") == 0)
  {
    *myVarNCData = malloc(sizeof(varNCData));
    (*myVarNCData)->varName = varName;
    (*myVarNCData)->ncType = NC_DOUBLE;
    ...
    int surfalbVarID;
    int res = nc_def_var(clmIDs[0], varName, (*myVarNCData)->ncType, (*myVarNCData)->dimSize,
                         (*myVarNCData)->dimIDs, &surfalbVarID);
    if (res != NC_ENAMEINUSE)
    {
      char *switch_name;
      char key[IDB_MAX_KEY_LEN];
      ...
      if (strcmp(switch_name, default_val) != 0)
      {
        ...
        chunksize[1] = GetInt("NetCDF.ChunkY");
        chunksize[2] = GetInt("NetCDF.ChunkX");
        nc_def_var_chunking(clmIDs[0], surfalbVarID, NC_CHUNKED, chunksize);
      }
    }
    if (res == NC_ENAMEINUSE)
    {
      res = nc_inq_varid(clmIDs[0], varName, &surfalbVarID);
    }
    return surfalbVarID;
  }

  if (strcmp(varName, "snowage_out") == 0)
  {
    *myVarNCData = malloc(sizeof(varNCData));
    (*myVarNCData)->varName = varName;
    (*myVarNCData)->ncType = NC_DOUBLE;
    ...
    int snowage_VarID;
    int res = nc_def_var(clmIDs[0], varName, (*myVarNCData)->ncType, (*myVarNCData)->dimSize,
                         (*myVarNCData)->dimIDs, &snowage_VarID);
    if (res != NC_ENAMEINUSE)
    {
      char *switch_name;
      char key[IDB_MAX_KEY_LEN];
      ...
      if (strcmp(switch_name, default_val) != 0)
      {
        ...
        chunksize[1] = GetInt("NetCDF.ChunkY");
        chunksize[2] = GetInt("NetCDF.ChunkX");
        nc_def_var_chunking(clmIDs[0], snowage_VarID, NC_CHUNKED, chunksize);
      }
    }
    if (res == NC_ENAMEINUSE)
    {
      res = nc_inq_varid(clmIDs[0], varName, &snowage_VarID);
    }
    return snowage_VarID;
  }

  if (strcmp(varName, "ndvi_out") == 0)
  {
    *myVarNCData = malloc(sizeof(varNCData));
    (*myVarNCData)->varName = varName;
    (*myVarNCData)->ncType = NC_DOUBLE;
    ...
    if (res != NC_ENAMEINUSE)
    {
      ...
      if (strcmp(switch_name, default_val) != 0)
      {
        ...
        chunksize[1] = GetInt("NetCDF.ChunkY");
        chunksize[2] = GetInt("NetCDF.ChunkX");
        nc_def_var_chunking(clmIDs[0], ndviVarID, NC_CHUNKED, chunksize);
      }
    }
    if (res == NC_ENAMEINUSE)
    {
      res = nc_inq_varid(clmIDs[0], varName, &ndviVarID);
    }
    return ndviVarID;
  }

  if (strcmp(varName, "qflx_snomelt_out") == 0)
  {
    *myVarNCData = malloc(sizeof(varNCData));
    (*myVarNCData)->varName = varName;
    (*myVarNCData)->ncType = NC_DOUBLE;
    ...
    (*myVarNCData)->dimIDs[1] = clmIDs[3];
    (*myVarNCData)->dimIDs[2] = clmIDs[4];
    int qflx_snomeltVarID;
    int res = nc_def_var(clmIDs[0], varName, (*myVarNCData)->ncType, (*myVarNCData)->dimSize,
                         (*myVarNCData)->dimIDs, &qflx_snomeltVarID);
    if (res != NC_ENAMEINUSE)
    {
      char *switch_name;
      char key[IDB_MAX_KEY_LEN];
      ...
      if (strcmp(switch_name, default_val) != 0)
      {
        ...
        chunksize[1] = GetInt("NetCDF.ChunkY");
        chunksize[2] = GetInt("NetCDF.ChunkX");
        nc_def_var_chunking(clmIDs[0], qflx_snomeltVarID, NC_CHUNKED, chunksize);
      }
    }
    if (res == NC_ENAMEINUSE)
    {
      res = nc_inq_varid(clmIDs[0], varName, &qflx_snomeltVarID);
    }
    return qflx_snomeltVarID;
  }
